...
We filter the data to datasets that have recorded growth rates `flag_growth == 1`, do not have genetic perturbations  `genetic_perturbations == 0`, are in MG1655 because our particular REL606 isn't in there, and occur in exponential phase/mid-log as opposed to the other unique things that occur in the tim_min column. 
```{r}
rates.only <- g.rates %>%
  filter(
    flag_growth == 1 &
      genetic_perturbations == 0 &
      strain == "MG1655" &
      time_min %in% c("mid-log phase", "exponential")
  ) %>%
  select(
    cel_file_name,
    geo_accession,
...
```{r}
rates.expr <-
  right_join(expr.data2, rates.only, by = "cel_file_name") %>%
  filter(!is.na(growth_rate_1h) & (!is.na(id) | !is.na(gene))) %>%
  select(-cel_file_name)
...
Somehting is up though, what is with all the 1's and near near 2 and below .5, looks like it's not truly continuous? It seems to be continuous in the non-WT data...
```{r}
rates.expr %>% 
  ggplot(., aes(growth_rate_1h))+
