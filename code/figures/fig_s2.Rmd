...
pc.big <- pc.df2 %>% 
  ...
  theme(text = element_text(size = 14),
        panel.grid = element_blank(),
        legend.background = element_blank(),
        legend.position = c(.72, .29),
        legend.text = element_text(size = 9),
        legend.key.size = unit(2, "mm"))+
  labs(x = "Number of lines",
       y = "Number of genes")

pc.bottom <- pc.big + inset_element(inset, left = .2, right = .95, top = .99, bottom = .4)
...
pd <- ddf %>% 
  ...
  geom_boxplot(outlier.size = .5)+
  geom_text(inherit.aes = FALSE, data = fc.count.df, aes(x = line, y = -.1, color = direc, label = n), position = position_dodge(width = .85), size = 3.5)+
...
pf <- hm.df %>%
  split(.$pc) %>%
  map(function(x) {
    ggplot(x, aes(line, target_id, fill = newfc, label = stars)) +
      geom_raster() +
      ...
      geom_text() +
      facet_wrap( ~ pc, ncol = 1, scales = "free") +
      theme_bw() +
      theme(
...
```{r fig.width = 11/3, fig.height = 3}
t1 <- all.data %>%
  filter(target_id %in% c(top.10.pc1)) %>% 
  select(target_id, rel_name) %>% 
  unique() %>% 
  mutate(target_id = factor(target_id, levels = top.10.pc1)) %>% 
  arrange(target_id) %>% 
  select(-target_id) %>% 
  rename("PC1 genes" = "rel_name") %>% 
...
t2 <- all.data %>%
  ...
  select(target_id, rel_name) %>% 
  unique() %>% 
  mutate(target_id = factor(target_id, levels = top.10.pc2)) %>% 
  arrange(target_id) %>% 
  select(-target_id) %>% 
  rename("PC2 genes" = "rel_name") %>% 
  tableGrob(theme = ttheme_minimal(), rows = NULL)
```
...
bottom <- (pd | pe | t1 | t2) + plot_layout(widths = c(.35, .35, .15, .15))
...
bottom <- (pd | pe | pf.full) + plot_layout(widths = c(.38, .32, .3))
...
ggsave(ff, filename = "../../figures/fig_s2.pdf", width = 11, height = 8)

ggsave(ff, filename = "../../figures/fig_s2.png", width = 11, height = 8)
```
