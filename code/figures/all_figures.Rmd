...
```
Figure S1: **(A)** The average number of reads aligned per gene by Kallisto for each sample. The color scheme remains the same in panels B and D. **(B)** Distributions of reads per gene in each sample. **(C)** Correlations between the replicates based on rounded counts or TPMs. 
**(D)** The periodicity of the ribo-seq datasets determined using a fast Fourier transform (see Methods M12). \newpage
...
Figure S5: **(A)** The relationship between the number of molecules of ERCC oligos added and their TPM in each sample. **(B)** The mean number of molecules per cell volume from all 4 ancestral samples compared to the mean from 2 replicates of an evolved line.\newpage
