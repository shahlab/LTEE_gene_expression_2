...
pb <- bdf2 %>%
  ...
  labs(x = "Mean ancestral TPM",
       ...
       title = "B")+
  stat_cor(size = 2.6, label.y.npc = .16, p.accuracy = .01, r.accuracy = .01)+
...
ff <- (pa / pb) + plot_layout(heights = c(.4, .6))
...
```{r}
ggsave(ff, filename = "../../figures/fig_s3.png", width = 11, height = 7)

ggsave(ff, filename = "../../figures/fig_s3.pdf", device = cairo_pdf, width = 11, height = 7)
